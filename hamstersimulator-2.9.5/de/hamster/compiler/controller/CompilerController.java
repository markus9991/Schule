...
public class CompilerController implements ActionListener {
	...
	public void actionPerformed(ActionEvent e) {
		if (e.getActionCommand() == CompilerController.ACTION_COMPILE) {
		...
		} else if(e.getActionCommand()==CompilerController.ACTION_FORMAT){
			Formatter formatter =new Formatter();
			File f=this.activeFile.getFile();
			...
			content+="}";
			try {
				content=formatter.formatSource(content);
			} catch (FormatterException e1) {
				// TODO Auto-generated catch block
				e1.printStackTrace();
			}
			System.out.println(content);
			content=content.replaceAll("class x \\{", "");
			System.out.println("===");
